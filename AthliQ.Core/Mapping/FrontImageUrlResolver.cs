...
{
	internal class FrontImageUrlResolver(IConfiguration _configuration) : IValueResolver<Child, GetChildDetailsDto, string>
	{
		public string Resolve(Child source, GetChildDetailsDto destination, string destMember, ResolutionContext context)
		{
			var baseUrl = _configuration.GetSection("Urls")["BaseUrl"];
			if (string.IsNullOrEmpty(source.ImageFrontURL))
				return string.Empty;
			return $"{baseUrl}Images/{source.ImageFrontURL}";
		}
	}
}
