...
namespace AthliQ.Service.Services.Children
{
	public class ChildService : IChildService
	{
		...
		public async Task<GenericResponse<ChildTestsGrades>> GetChildTestGradesAsync(int childId)
		{
			...
			var listOfScores = child.ChildTests.OrderBy(ct => ct.TestId)
											   .Select(ct => ct.TestResult)
											   .ToList();
			...
			var integrationResult = JsonSerializer.Deserialize<List<JavaChildTestGradesDto>>(ChildResult, jsonPolicy);
			...
			var result = integrationResult.Select(javaTest => new TestGradesDto
			{
		...
		public async Task<GenericResponse<GetChildDetailsDto>> ViewChildAsync(int childId)
		{
			...
			var preferedSportAr = await _unitOfWork.Repository<Sport, int>()
												   .Get(s => s.Id == child.SportPreferenceId).Result
			...
			var parentSportHistoryAr = await _unitOfWork.Repository<Sport, int>()
														.Get(s => s.Id == child.ParentSportHistoryId).Result
														.Select(s => s.ArabicName)
														.FirstOrDefaultAsync();
